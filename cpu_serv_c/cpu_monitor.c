...
static void cpuset_push(CpuSet *s, const char *label, CpuTimes t) {
    if (s->count == s->cap) {
        ...
        CpuLine *tmp = realloc(s->arr, ncap * sizeof(CpuLine));
...
int cpu_monitor_get_usage(CpuUsageData *data) {
    ...
    if (data->core_count > 0) {
        data->core_usage = malloc(data->core_count * sizeof(double));
        if (!data->core_usage) {
            pthread_mutex_unlock(&data_mutex);
            return -1;
        }
...
static void* udp_client_thread(void* arg) {
    ...
    while (udp_running) {
        ...
        if (cpu_monitor_get_usage(&data) == 0) {
            ...
            for (size_t i = 0; i < data.core_count && len < sizeof(buffer) - 10; i++) {
